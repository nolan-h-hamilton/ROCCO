"""
Runs multiple `ROCCO_chrom.py` jobs, which execute the ROCCO peak caller
algorithm on each respective chromosome. Chromosome-specific parameters 
...
If any parameter entry in the CSV file is set to `NULL`, the script uses the
...
See `demo.ipynb` for a more complete walkthrough.


a) Run ROCCO with parameters specified in CSV file `params.csv`, store results in `output_dir`
and combine peaks into single bed file `ROCCO_combined_out.bed`. 
```
python3 ROCCO.py -p params.csv --outdir output_dir --combine ROCCO_combined_out.bed 
```

b) Run ROCCO with parameters specified in CSV file `params.csv`, store results in `output_dir`
and combine peaks into single bed file `ROCCO_combined_out.bed`. Run `ROCCO_chrom.py` jobs
in parallel (two at a time) and use the MOSEK solver:
```
python3 ROCCO.py -p params.csv --outdir output_dir --combine ROCCO_combined_out.bed --solver MOSEK --jobs 2
```

c) Run ROCCO with parameters specified in CSV file `params.csv`, store results in `group_one_dir`
and combine peaks into single bed file `group_one_out.bed`. Run `ROCCO_chrom.py` jobs in parallel
(two at a time) and use the MOSEK solver:
```
python3 ROCCO.py -p params.csv --outdir group_one_dir --combine group_one_out.bed --solver MOSEK --jobs 2
```
