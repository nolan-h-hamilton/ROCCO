"""
This script executes multiple `ROCCO_chrom.py` jobs, which run the ROCCO
algorithm on each respective chromosome. Chromosome-specific parameters 
are collected a CSV file specified with argument `-p --param_file`. Consider
...
Note: The default solver is ECOS, which is free and open-source, and is installed
alongside cvxpy. However, using the MOSEK solver can improve speed.
Free academic licenses are available for MOSEK: https://www.mosek.com/products/academic-licenses/
After obtaining a license, a simple `pip install mosek` should allow you to specify `--solver MOSEK`
at the command line when using `ROCCO.py`.
...
Options:
  ...
  -N RR_ITER, --rr_iter RR_ITER
                        Set the number of random restart iterations (default: 50)
  ...
  --identifiers IDENTIFIERS
                        File containing identifiers for samples to include in the experiment
...
def sort_combine_bed(outfile: str, dir_: str = '.') -> None:
    """
