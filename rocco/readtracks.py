...
def _check_read(read: pysam.AlignedSegment):
...
def score_peaks(bam_files, chrom_sizes_file: str=None,
                ...
                threads: int=None, pc=1, ecdf_nsamples=250,
    ...
    ecdf_dict = multi_ecdf(bam_files, lengths, chrom_sizes_file, nsamples_per_length=ecdf_nsamples, sample_scaling_constants=sample_scaling_constants)
...
def _check_type_bam_files(bam_files):
    if isinstance(bam_files, str):
        with open(bam_files, 'r') as f:
            bam_files_ = [line.strip() for line in f if line.strip()]
        for file_ in bam_files_:
        ...
        raise ValueError("bam_files must be either a list or a path to a file containing a list of BAM file paths.")
    return bam_files_


def get_ecdf(bam_files, length: int, chrom_sizes_file: str,
             nsamples=250, sample_scaling_constants: list = None, seed: int=None):
    bam_files_ = _check_type_bam_files(bam_files)
    len_avgs = []
    ...
    with subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, encoding='utf-8', bufsize=1) as proc:
        ...
        for line in proc.stdout:
            ...
            sample_avgs.extend(cperlen)
    len_avgs = np.array(sample_avgs)
...
def multi_ecdf(bam_files, lengths, chrom_sizes_file: str,
    ...
    r"""Compute an empirical cdf of counts from randomly selected genomic regions matching peak lengths

    :param bam_files: List of paths to the BAM files OR a single filepath to a text file containing a list of BAM files (one filepath per line).
    :type bam_files: list or str
    :param lengths: List of peak lengths.
    :type lengths: list
    :param chrom_sizes_file: chromosome sizes file.
    :type chrom_sizes_file: str
    :param nsamples_per_length: Number of samples to generate for each length.
    """
    bam_files_ = _check_type_bam_files(bam_files)
