...
Procedure:
    For each chromosome:
        1. randomly select a fraction, `--samp_rate`, of loci in the chromosome (i.e., randomly choose a set of indices/nucleotide positions)
        1. Compute the average signal value at each selected locus with respect to each sample/replicate's wig file.
            Denote the set of such values as `chrom_loci_avgs`
        1. Calculate the average signal value across all sampled loci, i.e., take the mean of values in `chrom_loci_avgs`
            Denote as `est_chrom_density`
            
    With `est_chrom_density` recorded for each chromosome,
        1. Scale the `est_chrom_density` values such that they average to `--smean`
